...
export default function PricingTime({ timeBased, setTimeBased }) {
  ...
  function AddTime() {
    const usedAllocations = amount.pricing.map((p) => p.time_allocation);
    const available = newSlotButtons.find(
      (time) => !usedAllocations.includes(time)
    );
    if (!available) return;
  ...
  function Submit() {
    const filledPricing = amount.pricing
      .filter(
        (slot) =>
    ...
    // ✅ Set ONLY the pricing array into your timeBased context/state
    setTimeBased((prev) => ({
      ...prev,
      pricing: filledPricing,
    }));
    ...
    setAmount({
      pricing: [{ time_allocation: "15", virtual: "", in_person: "" }],
    });
    setIsAddTime([]);
  ...
  // Validation logic
  const isNext = amount.pricing.some(
    (entry) =>
      (entry.virtual ?? "").trim() !== "" ||
          ...
          <div className={styles.timecontainer}>
            ...
            <div className={styles.addtimeslot}>
              {IsAddTime.map((slot, index) => {
                // Calculate the corresponding index in the amount.pricing array
                // since the first slot is hardcoded at amount.pricing[0]
                const pricingIndex = index + 1;
                return (
                  <div key={slot.id} className='time-slot'>
                    <div className={styles.inputtimecontainer}>
                      <div className={styles.buttonmins}>
                        {/* Iterate over slot.buttons to create selectable time options */}
                        {/* Only using the first button value here, if you want all to be selectable,
                            you'd map over slot.buttons and create a button for each */}
                        <button
                        ...
                        >
                          <div>
                            ...
                            <span>
                              {amount.pricing[pricingIndex]?.time_allocation ||
                                slot.buttons[0]}
                            </span>{" "}
                            {/* Display selected time or default from slot */}
                          </div>
                        </button>
                        <span>Min</span>
                      </div>
                      ...
                      <div className={styles.nairavirtual}>
                        <input
                          ...
                          name='virtual'
                          placeholder='₦20,000'
                          value={amount.pricing[pricingIndex]?.virtual || ""}
                          onChange={(e) => Details(e, pricingIndex, "virtual")}
                        />
                      </div>
                      ...
                      <div className={styles.nairainperson}>
                        <input
                          ...
                          name='in_person'
                          placeholder='₦20,000'
                          value={amount.pricing[pricingIndex]?.in_person || ""}
                          onChange={(e) =>
                            Details(e, pricingIndex, "in_person")
                          }
                        />
                      </div>
                      <svg
